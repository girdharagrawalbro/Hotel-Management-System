...
<?php
    ...
    $month =date("m");
    $thismonth = "2023-$month-01";
    $todaydate = date("y/m/d");

    //roombook roomtype
    $chartroom1 = "SELECT * FROM roombook WHERE RoomType='Superior' and BOOKDATE between '$thismonth' and '$todaydate'";
    $chartroom1re = mysqli_query($conn, $chartroom1);
    $chartroom1row = mysqli_num_rows($chartroom1re);

    $chartroom2 = "SELECT * FROM roombook WHERE RoomType='Deluxe' and BOOKDATE between '$thismonth' and '$todaydate'";
    $chartroom2re = mysqli_query($conn, $chartroom2);
    $chartroom2row = mysqli_num_rows($chartroom2re);

    $chartroom3 = "SELECT * FROM roombook WHERE RoomType='Guest' and BOOKDATE between '$thismonth' and '$todaydate'";
    $chartroom3re = mysqli_query($conn, $chartroom3);
    $chartroom3row = mysqli_num_rows($chartroom3re);

    $chartroom4 = "SELECT * FROM roombook WHERE RoomType='Single' and BOOKDATE between '$thismonth' and '$todaydate'";
